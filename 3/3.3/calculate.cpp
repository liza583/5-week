...
void calculateTemperatures(const string& input, SensorData sensors[], int& sensorCount, bool& hasErrors) {
    ...
    try {
        ...
        const size_t MAX_LENGTH = 256; // Максимальная длина строки
        ...
        while (getline(ss, token, '@')) {
            if (token.length() < 4) {
                throw std::runtime_error("Ошибка: Неверный формат данных."); // Заменяем на исключение
            }
            ...
            if (temperature > 50 || temperature < -50) {
                throw TemperatureOutOfRangeException(temperature);
            }
        ...
        cerr << e.what() << endl;
        hasErrors = true;
    } catch (const TemperatureOutOfRangeException& e) {
        cerr << e.what() << endl;
        hasErrors = true;
    } catch (const MaxLengthExceededException& e) {
        cerr << e.what() << endl;
        hasErrors = true;
